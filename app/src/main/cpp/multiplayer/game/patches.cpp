...
void ApplyPatches()
{
	...
	CHook::RET("_Z16SaveGameForPause10eSaveTypesPc"); // íå ñîõðàíÿòü ïðè ñâîðà÷èâàíèè. ÷åðíûé ýêðàí
	...
	CHook::RET("_ZN12CAudioEngine16StartLoadingTuneEv"); // çâóê çàãðóçî÷íîãî ýêðàíà
	...
	CHook::NOP(g_libGTASA + 0x00584884, 2);	// íå äàâàòü ãàí ïðè âûõîäå èç òà÷êè 	( êëþøêà, äðîáîâèê and etc )
	CHook::NOP(g_libGTASA + 0x00584850, 2);	// íå äàâàòü ãàí ïðè âûõîäå èç òà÷êè	( êëþøêà, äðîáîâèê and etc )
#else
    CHook::NOP(g_libGTASA + 0x004D8700, 1);  // CCoronas::RenderSunReflection crash
    CHook::NOP(g_libGTASA + 0x006A852C, 1);  // íå äàâàòü ãàí ïðè âûõîäå èç òà÷êè   ( êëþøêà, äðîáîâèê and etc )
    CHook::NOP(g_libGTASA + 0x006A84E0, 1);  // íå äàâàòü ãàí ïðè âûõîäå èç òà÷êè  ( êëþøêà, äðîáîâèê and etc )
	...
	CHook::RET("_ZN4CHud23DrawBustedWastedMessageEv"); // CHud::DrawBustedWastedMessage // ÏÎÒÐÀ×ÅÍÎ
...
//	CHook::WriteMemory(g_libGTASA + 0x00341F84, (uintptr_t)"\x00\xF0\x21\xBE", 4);
	...
	// êàðòà â ìåíþ
	CHook::NOP(g_libGTASA + (VER_x32 ? 0x2ABA08 : 0x36A6E8), 2); // òåêñò ëåãåíäû êàðòû
	CHook::NOP(g_libGTASA + (VER_x32 ? 0x2ABA14 : 0x36A6F8), 2); // çíà÷êè ëåãåíäû
    CHook::NOP(g_libGTASA + (VER_x32 ? 0x2AB4A6 : 0x36A190), 2); // íàçâàíèå ìåñòíîñòè
}
